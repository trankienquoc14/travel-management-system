...
const regexPassenger = /<\/div>\s*\{\/\*\s*Detail Modal Footer\s*\*\/\}/;

const passengerBlock = `
                            {/* Khối Thông Tin Khách Hàng */}
                            {!detailBooking.isService && (
                                <div style={{ background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '20px', padding: '20px', marginBottom: '24px', margin: '0 32px 32px 32px' }}>
...
if (code.match(regexPassenger)) {
    code = code.replace(regexPassenger, passengerBlock);
    fs.writeFileSync('frontend/src/components/MyBookings.jsx', code, 'utf8');
    console.log('Injected Passenger Info');
} else {
    console.log('Missed Passenger Info');
}
