...
// Lưu bản thiết kế Tour Cố Định từ UI của Staff
router.post('/design', protect, upload.single('image'), tourController.saveFixedTourDesign);

// --- STAFF ROUTES ---
router.get('/staff/tours', tourController.getAllFixedTours);
router.get('/staff/tours/:id', protect, tourController.getFixedTourById);
router.get('/staff/destination-resources', protect, tourController.getDestinationResources);

// Lấy chi tiết lịch trình vận hành để vào form 3 Tab (Admin)
...
// Route dùng để Quản lý điều chỉnh giá nhanh
router.put('/admin/price/:id', protect, tourController.updateTourPrice);
