...
if (headerIdx !== -1) {
    ...
    // We just find the END of the div that contains {formatCurrency(detailBooking.total_amount)}
    // which is the end of the original block.
    const currencyStr = '{formatCurrency(detailBooking.total_amount)}</strong>';
    const currencyIdx = code.indexOf(currencyStr, startIdx);
    
    // Find the closing divs for this block.
    const closingDivsStr = `</div>
                                    </div>`;
    const endIdx = code.indexOf(closingDivsStr, currencyIdx);
    ...
    if (startIdx !== -1 && endIdx !== -1) {
        ...
        code = code.substring(0, startIdx) + replacement + code.substring(endIdx + closingDivsStr.length);
