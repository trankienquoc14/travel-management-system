...
function replaceInFile(fileName, searchStart, searchEnd) {
    const fPath = path.join(__dirname, 'src', 'components', fileName);
    let content = fs.readFileSync(fPath, 'utf8');

    const startIdx = content.indexOf(searchStart);
    if (startIdx === -1) {
        console.log(\`Could not find searchStart in \${fileName}\`);
        return;
    }
    const endIdx = content.indexOf(searchEnd, startIdx);
    if (endIdx === -1) {
        console.log(\`Could not find searchEnd in \${fileName}\`);
        return;
    }
    ...
    // For StaffPendingTours and ManagerTourApproval, the original 💰 Cơ cấu giá bán block shouldn't be rendered because 
    // we now show everything in our new dashboard layout.
    // So we will delete the 💰 Cơ cấu giá bán header too by starting searchStart from there.
    
    fs.writeFileSync(fPath, before + extractDashboardJSX + '\n' + after, 'utf8');
    console.log(\`Fixed \${fileName}\`);
}

replaceInFile(
    'StaffPendingTours.jsx', 
    '<h4 style={{ margin: \\'0 0 12px 0\\', fontSize: \\'16px\\', color: \\'#1e293b\\' }}>💰 Cơ cấu giá bán</h4>',
    '                                            </div>\n                                        );\n                                    } catch (e) { return <span>Lỗi hiển thị lịch trình.</span>; }'
);

replaceInFile(
    'ManagerTourApproval.jsx',
    '<h4 style={{ margin: \\'0 0 12px 0\\', fontSize: \\'16px\\', color: \\'#1e293b\\' }}>💰 Cơ cấu giá bán</h4>',
