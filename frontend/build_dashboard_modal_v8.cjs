...
function doReplace(fileName, startString, endString, replacerVar) {
    const fPath = path.join(__dirname, 'src', 'components', fileName);
    let content = fs.readFileSync(fPath, 'utf8');

    const startIndex = content.indexOf(startString);
    if (startIndex === -1) {
        console.log("Could not find startString in " + fileName);
        return;
    }

    const endIndex = content.indexOf(endString, startIndex);
    if (endIndex === -1) {
        console.log("Could not find endString in " + fileName);
        return;
    }
    ...
    let replacement = correctJSXStaff.replace(/viewingFixedTour/g, replacerVar);

    fs.writeFileSync(fPath, before + replacement + '\n                        ' + after, 'utf8');
...
doReplace(
    ...
    "{viewingFixedTour.status === 'Rejected'",
    "viewingFixedTour"
);
