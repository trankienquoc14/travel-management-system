...
const filePath = path.join(__dirname, 'src', 'components', 'ManagerTourApproval.jsx');
let content = fs.readFileSync(filePath, 'utf8');

const regex = /return \(\s*<div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>[\s\S]*?<\/div>\s*\);\s*\} catch \(e\) \{ return <span>Lỗi hiển thị lịch trình.<\/span>; \}/g;

const matches = content.match(regex);
if (matches && matches.length === 1) {
    content = content.replace(regex, newRenderLogic);
    fs.writeFileSync(filePath, content, 'utf8');
    console.log('Updated ManagerTourApproval.jsx');
} else {
    console.log('Regex matched ' + (matches ? matches.length : 0) + ' times in ManagerTourApproval.jsx');
}
