...
const startLoop = `{/* Lịch trình từng ngày theo timeline */}`;
const endLoop = `                                {/* Xử lý cho định dạng mới (days) */}`;
const finalEndLoop = `                                    </div>
                                ))}
                            </div>`;
...
for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes(startLoop)) {
        skip = true;
        newLines.push(`                            {/* Lịch trình từng ngày theo timeline */}
    ...
    if (skip && lines[i].includes('</div>') && lines[i-1] && lines[i-1].includes('))}')) {
        if(lines[i-2] && lines[i-2].includes('</div>')) {
            skip = false;
            continue;
        }
    }
    // We need a better way to slice it
}
`
