const fs = require('fs');
let c = fs.readFileSync('src/components/TourDetail.jsx', 'utf8').replace(/\r\n/g, '\n');
...
// 2. Add getDestString right above handleGoToBooking
c = c.replace(
...
// 3. Update fetchTourDetail to also fetch destinations
// We will replace the whole fetchTourDetail block carefully.
const oldFetchTourDetail = `    const fetchTourDetail = async () => {
        try {
...
const newFetchTourDetail = `    const fetchTourDetail = async () => {
        try {
...
c = c.replace(oldFetchTourDetail, newFetchTourDetail);

// 4. Update the layout: Hero banner
c = c.replace(
...
// 6. Update Categories and Highlights (Overview)
const oldOverview = `<h2>Tổng quan chuyến đi</h2>
