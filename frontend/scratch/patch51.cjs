...
const sIdx = c.indexOf("                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(380px, 1fr))', gap: '20px' }}>");
const eIdx = c.indexOf("                                    </div>\n                                )}");

if (sIdx > -1 && eIdx > -1) {
    const toReplace = c.substring(sIdx, eIdx + 42); // 42 is length of "                                    </div>"
    ...
    console.log('Not found boundaries', sIdx, eIdx);
}
