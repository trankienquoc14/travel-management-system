...
function doReplaceStaffPending() {
    const file = 'StaffPendingTours.jsx';
    const fPath = path.join(__dirname, 'src', 'components', file);
    let content = fs.readFileSync(fPath, 'utf8');

    // Regex to match from 💰 Cơ cấu giá bán to the end of TIMELINE LỊCH TRÌNH
    // It is bounded by: <h4 style={{ margin: '0 0 12px 0', fontSize: '16px', color: '#1e293b' }}>💰 Cơ cấu giá bán</h4>
    // Down to: </div> (closing the Lịch trình chi tiết wrapper)
    const regex = /<h4[^>]*>💰 Cơ cấu giá bán<\/h4>[\s\S]*?(?=<\/div>\s*<\/div>\s*\{viewingFixedTour\.status === 'Rejected')/m;
    
    let replacement = extractDashboardJSX;
...
function doReplaceManagerApproval() {
    const file = 'ManagerTourApproval.jsx';
    const fPath = path.join(__dirname, 'src', 'components', file);
    let content = fs.readFileSync(fPath, 'utf8');

    // Very similar regex but for ManagerTourApproval
    const regex = /<h4[^>]*>💰 Cơ cấu giá bán<\/h4>[\s\S]*?(?=<\/div>\s*<\/div>\s*\{isRejecting &&)/m;
...
doReplaceStaffPending();
doReplaceManagerApproval();
