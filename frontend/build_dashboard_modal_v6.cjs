...
function replaceRegexInFile(fileName) {
    const fPath = path.join(__dirname, 'src', 'components', fileName);
    let content = fs.readFileSync(fPath, 'utf8');

    // This regex looks for `<h4 ...>💰 Cơ cấu giá bán</h4>`
    // And matches everything up to `} catch (e) { return <span>Lỗi hiển thị lịch trình.</span>; }`
    // OR up to the end of the IIFE `})()}` right before the closing `</div>` of the map area.
    
    const regex = /<h4[^>]*>💰 Cơ cấu giá bán<\/h4>[\s\S]*?\} catch \(e\) \{ return <span[^>]*>Lỗi hiển thị lịch trình\.<\/span>; \}\r?\n?\s*\}\)\(\)\}/m;
    
    if (regex.test(content)) {
        content = content.replace(regex, extractDashboardJSX);
        fs.writeFileSync(fPath, content, 'utf8');
        console.log("Fixed " + fileName);
    } else {
        console.log("Regex not matched in " + fileName);
    }
}

replaceRegexInFile('ManagerTourApproval.jsx');
